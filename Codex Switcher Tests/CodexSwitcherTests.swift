...
struct CodexSwitcherTests {
    ...
    @Test func captureCurrentAccountSupportsUndoAndRedoWhenUndoManagerConfigured() async throws {
        let container = try makeInMemoryContainer()
        let undoManager = UndoManager()
        let controller = makeController(
            authFileManager: FakeAuthFileManager(contents: makeChatGPTAuthJSON(accountID: "acct-undo-add"))
        )

        controller.configure(modelContext: container.mainContext, undoManager: undoManager)

        await controller.captureCurrentAccountNow()
        container.mainContext.processPendingChanges()

        let savedAccount = try #require(fetchAccounts(in: container.mainContext).first)
        ...
        let restoredAccount = try #require(fetchAccounts(in: container.mainContext).first)
        #expect(restoredAccount.identityKey == savedAccount.identityKey)
        #expect(restoredAccount.name == savedAccount.name)
    }

    @Test func renamingAccountSupportsUndoAndRedoWhenUndoManagerConfigured() throws {
        let container = try makeInMemoryContainer()
        let undoManager = UndoManager()
        let controller = makeController(
            authFileManager: FakeAuthFileManager(contents: makeChatGPTAuthJSON(accountID: "acct-undo-rename"))
        )

        controller.configure(modelContext: container.mainContext, undoManager: undoManager)

        let account = makeStoredAccount(name: "Original", customOrder: 0, accountID: "acct-undo-rename")
        container.mainContext.insert(account)
        try container.mainContext.save()

        controller.commitRename(for: account.id, proposedName: "Renamed")
        container.mainContext.processPendingChanges()

        let renamedAccount = try #require(fetchAccounts(in: container.mainContext).first)
        #expect(renamedAccount.name == "Renamed")
        #expect(undoManager.canUndo)

        undoManager.undo()

        let originalAccount = try #require(fetchAccounts(in: container.mainContext).first)
        #expect(originalAccount.name == "Original")
        #expect(undoManager.canRedo)

        undoManager.redo()

        let redoneAccount = try #require(fetchAccounts(in: container.mainContext).first)
        #expect(redoneAccount.name == "Renamed")
    }

    @Test func modelUndoControllerExposesRenameUndoAvailabilityAndTitles() throws {
        let container = try makeInMemoryContainer()
        let undoManager = UndoManager()
        let modelUndoController = ModelUndoController()
        let controller = makeController(
            authFileManager: FakeAuthFileManager(contents: makeChatGPTAuthJSON(accountID: "acct-undo-menu"))
        )

        controller.configure(modelContext: container.mainContext, undoManager: undoManager)
        modelUndoController.configure(modelContext: container.mainContext, undoManager: undoManager)

        let account = makeStoredAccount(name: "Original", customOrder: 0, accountID: "acct-undo-menu")
        container.mainContext.insert(account)
        try container.mainContext.save()
        ...
        #expect(modelUndoController.canUndo)
        #expect(modelUndoController.undoMenuTitle == undoManager.undoMenuItemTitle)

        modelUndoController.undo()

        let restoredAccount = try #require(fetchAccounts(in: container.mainContext).first)
        #expect(restoredAccount.name == "Original")
        #expect(modelUndoController.canRedo)
        #expect(modelUndoController.redoMenuTitle == undoManager.redoMenuItemTitle)
    }

    @Test func modelUndoControllerPersistsUndoAndRedoForDeletedAccounts() async throws {
        let temporaryDirectory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: temporaryDirectory) }

        let container = try makePersistentContainer(at: temporaryDirectory)
        let undoManager = UndoManager()
        let modelUndoController = ModelUndoController()
        let controller = makeController(
            authFileManager: FakeAuthFileManager(contents: makeChatGPTAuthJSON(accountID: "acct-delete-undo"))
        )
        let deletedAccount = makeStoredAccount(name: "Deleted", customOrder: 0, accountID: "acct-delete-undo")
        let remainingAccount = makeStoredAccount(name: "Remaining", customOrder: 1, accountID: "acct-delete-keep")

        container.mainContext.insert(deletedAccount)
        container.mainContext.insert(remainingAccount)
        try container.mainContext.save()
        ...
        #expect(
            await controller.removeAccountsNow(
                withIDs: [deletedAccount.id],
                allowsInteractiveRecovery: false
            )
        )
        container.mainContext.processPendingChanges()

        #expect(try fetchAccounts(in: container.mainContext).map(\.id) == [remainingAccount.id])
        #expect(undoManager.canUndo)
        ...
        #expect(try fetchAccounts(in: container.mainContext).count == 2)

        let restoredAccounts = try fetchAccounts(in: ModelContext(container))
        #expect(restoredAccounts.count == 2)
        #expect(Set(restoredAccounts.map(\.id)) == Set([deletedAccount.id, remainingAccount.id]))
        ...
        #expect(!container.mainContext.hasChanges)
        let reDeletedAccounts = try fetchAccounts(in: ModelContext(container))
        #expect(reDeletedAccounts.map(\.id) == [remainingAccount.id])
    }

    @Test func removeAccountsRunsSynchronouslyAfterInitializationWhenUndoManagerConfigured() async throws {
        let container = try makeInMemoryContainer()
        let undoManager = UndoManager()
        let controller = makeController(
            authFileManager: FakeAuthFileManager(contents: makeChatGPTAuthJSON(accountID: "acct-sync-delete"))
        )
        let account = makeStoredAccount(name: "Delete Me", customOrder: 0, accountID: "acct-sync-delete")
    ...
    @Test func invalidatingPendingAccountRemovalRequestsCancelsQueuedRemoval() async throws {
        let container = try makeInMemoryContainer()
        let undoManager = UndoManager()
        let controller = makeController(
            authFileManager: FakeAuthFileManager(contents: makeChatGPTAuthJSON(accountID: "acct-cancel-delete"))
        )
        let account = makeStoredAccount(name: "Keep Me", customOrder: 0, accountID: "acct-cancel-delete")
        ...
        #expect(remainingAccount.id == account.id)
    }
    ...
    @Test func removeAllAccountsSupportsUndoAndDefersSnapshotCleanupWhenUndoManagerConfigured() async throws {
        let container = try makeInMemoryContainer()
        let undoManager = UndoManager()
        let secretStore = FakeSecretStore()
        let syncedRateLimitCredentialStore = FakeSyncedRateLimitCredentialStore()
        ...
        let firstAccount = makeStoredAccount(name: "Work", customOrder: 0, accountID: "acct-undo-work")
        let secondAccount = makeStoredAccount(name: "Personal", customOrder: 1, accountID: "acct-undo-personal")

        container.mainContext.insert(firstAccount)
        container.mainContext.insert(secondAccount)
        try container.mainContext.save()

        try await secretStore.saveSnapshot("work-secret", forIdentityKey: firstAccount.identityKey)
        try await secretStore.saveSnapshot("personal-secret", forIdentityKey: secondAccount.identityKey)
        try await syncedRateLimitCredentialStore.save(
            SyncedRateLimitCredential(
                credentials: CodexRateLimitCredentials(
                    identityKey: firstAccount.identityKey,
                    authMode: .chatgpt,
                    accountID: "acct-undo-work",
                    accessToken: "token-work",
                    idToken: nil
                )
            )
        ...
        #expect(try fetchAccounts(in: container.mainContext).isEmpty)
        #expect(await secretStore.secret(forIdentityKey: firstAccount.identityKey) == "work-secret")
        #expect(await secretStore.secret(forIdentityKey: secondAccount.identityKey) == "personal-secret")
        #expect(await syncedRateLimitCredentialStore.containsCredential(forIdentityKey: firstAccount.identityKey))
        #expect(await syncedRateLimitCredentialStore.containsCredential(forIdentityKey: secondAccount.identityKey))

        undoManager.undo()

        let restoredAccounts = try fetchAccounts(in: container.mainContext)
        #expect(restoredAccounts.count == 2)
        #expect(
            Set(restoredAccounts.map(\.identityKey))
                == Set([firstAccount.identityKey, secondAccount.identityKey])
        )
    }
...
private func makePersistentContainer(at directoryURL: URL) throws -> ModelContainer {
    let schema = Schema([StoredAccount.self])
    let configuration = ModelConfiguration(
        "TestAccounts-\(UUID().uuidString)",
        schema: schema,
        url: directoryURL.appendingPathComponent("Accounts.sqlite"),
        allowsSave: true,
        cloudKitDatabase: .none
    )
    return try ModelContainer(for: schema, configurations: [configuration])
